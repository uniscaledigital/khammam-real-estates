    ...
    <>
      <section className="bg-secondary py-20 text-white">
        <div className="mx-auto max-w-7xl px-6">
          ...
          <p className="mt-4 text-white/80 font-sub text-lg">We respond to most enquiries within an hour during business hours.</p>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-6 py-16">
        ...
        <div className="grid gap-12 lg:grid-cols-2">
          ...
          <form onSubmit={(e) => { e.preventDefault(); mut.mutate(form); }} className="grid gap-6 rounded-[24px] border border-border bg-card p-8 shadow-xl shadow-black/5">
...
function InfoCard({ icon: Icon, title, lines }: { icon: any; title: string; lines: string[] }) {
  return (
    <div className="flex gap-5 rounded-[20px] border border-border bg-card p-6 shadow-sm transition-all hover:-translate-y-1 hover:shadow-md">
      <div className="grid h-14 w-14 shrink-0 place-items-center rounded-2xl bg-gradient-to-br from-primary/20 to-transparent text-primary"><Icon className="h-6 w-6 stroke-[1.5]" /></div>
      <div>
        <h3 className="font-display text-lg font-bold text-secondary mb-1">{title}</h3>
        {lines.map((l, i) => <p key={i} className="font-sub text-[15px] font-medium text-muted-foreground leading-relaxed">{l}</p>)}
      </div>
    </div>
  );
}
